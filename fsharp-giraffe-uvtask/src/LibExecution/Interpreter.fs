...
type Dval =
  ...
  member this.toJSON() : FSharp.Data.JsonValue =
    ...
    | DList l ->
      l |> List.map (fun dv -> dv.toJSON ()) |> List.toArray |> JsonValue.Array

    | DBool b -> JsonValue.Boolean b
    | DLambda _ -> JsonValue.Null
    | DSpecial (DError (e)) ->
      JsonValue.Record [| "error", JsonValue.String(e.ToString()) |]
...
and Param = { name : string; tipe : DType; doc : string }
...
module Environment =
  ...
  and AsyncFn =
    { name : FnDesc.T
      parameters : List<Param>
      returnVal : RetVal
      fn : (T * List<Dval>) -> ValueTask<Result<Dval, unit>> }

  and T = { functions : Map<FnDesc.T, BuiltInFn> }

  let envWith (functions : Map<FnDesc.T, BuiltInFn>) : T = { functions = functions }

let param (name : string) (tipe : DType) (doc : string) : Param =
  { name = name; tipe = tipe; doc = doc }

let retVal (tipe : DType) (doc : string) : Environment.RetVal =
  { tipe = tipe; doc = doc }
...
let map_s (list : List<'a>) (f : 'a -> ValueTask<'b>) : ValueTask<List<'b>> =
  uvtask {
    let! result =
      uvtask {
        let! (accum : List<'b>) =
          List.fold
            (fun (accum : ValueTask<List<'b>>) (arg : 'a) ->
              uvtask {
...
let rec evalAsync
  ...
  match e with
  | EInt i -> uvtask { return DInt i }
  | EString s -> uvtask { return DString s }
  | ELet (lhs, rhs, body) ->
    uvtask {
  ...
  | EFnCall (desc, args) ->
    (match tryFindFn desc with
     ...
     | None -> uvtask { return (err (NotAFunction desc)) })
  | EBinOp (arg1, desc, arg2) ->
    (match tryFindFn desc with
     ...
     | None -> uvtask { return (err (NotAFunction desc)) })
  | ELambda (vars, expr) -> uvtask { return DLambda(st, vars, expr) }
  | EVariable (name) -> uvtask { return Symtable.get st name }
    ...
    let fns : List<Environment.BuiltInFn> =
      [ Environment.Sync
          { name = (FnDesc.stdFnDesc "Int" "range" 0)
            ...
            returnVal =
              retVal (TList(TInt)) "List of ints between lowerBound and upperBound"
            fn =
              (function
...
let runAsync (e : Expr) : Task<Dval> =
  (evalAsync env Symtable.empty e).AsTask()
