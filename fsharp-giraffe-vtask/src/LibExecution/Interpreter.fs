...
let rec evalAsync
  ...
  match e with
  | EInt i -> vtask { return DInt i }
  | EString s -> vtask { return DString s }
  | ELet (lhs, rhs, body) ->
    vtask {
  ...
  | EFnCall (desc, args) ->
    (match tryFindFn desc with
     ...
     | None -> vtask { return (err (NotAFunction desc)) })
  | EBinOp (arg1, desc, arg2) ->
    (match tryFindFn desc with
     ...
     | None -> vtask { return (err (NotAFunction desc)) })
  | ELambda (vars, expr) -> vtask { return DLambda(st, vars, expr) }
  | EVariable (name) -> vtask { return Symtable.get st name }
